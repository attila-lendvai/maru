...
- Arch-specific code in the shared files (`cffi-late.l`, `elf-early.l`) must be guarded (`if-at-expand (= "x86" *compiler-backend*)`) or overridden per backend (e.g. `emit-entry-point-wrapper` in `emit-arm.l`).
