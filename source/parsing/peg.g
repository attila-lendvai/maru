...
`( 'match-rule .:name .+:args		-> `(let ((pos (<parser-stream>-position self.source)))
 ...
 | 'match-require (&(..) effect)*:e .:msg -> `(or ,@e (error "parser error (value); expected: " ,msg " at: " ($source-position self.source)))
...
`( 'match-rule .:name .+:args		-> `(let ((pos (<parser-stream>-position self.source)))
 ...
 | 'match-require (&(..) effect)*:e .:msg -> `(or ,@e (error "parser error (effect); expected: " ,msg " at: " ($source-position self.source)))
